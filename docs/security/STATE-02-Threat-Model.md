...
controls only: no risk is accepted, no provider budget is armed, no operational
ledger or sandbox is implemented, and no platform behaviour is proved. The
effective provider budget remains zero and `Disarmed`.
...
| `THR-S02-027` | Readiness calls external services repeatedly, rearms provider capability or leaks diagnostics. | Cost, outage amplification, recreated authority or information disclosure. | No billable probe or rearm; local durable budget/circuit state only; sanitised closed capability status without limits, account, schedule, actor or request identity. | S04/S07. | Architecture mitigated; implementation and runtime evidence open. |
...
| `THR-S02-035` | Unbounded catalogue additions exhaust storage, queue, parser or provider budget. | Denial of service or unexpected cost. | Per-operation limits; capacity gate and activation refusal without truncation; durable aggregate and `AdministrativeIndexEmbedding` allocations; maximum reservation before credential lookup; explicit rearm and new authority for any limit increase. | S03/S04/S07/S08. | ADR-0018 architecture accepted; persistence, capacity, budget arming and operational evidence open. |
...
residual risk as open until separately authorised implementation and evidence
exist. In particular, no provider budget is armed or priced, uncertain spend
